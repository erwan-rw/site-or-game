  ...
  <script>
    ...
    class LevelIntroScene extends Phaser.Scene {
      ...
      create(data) {
        ...
        const msg = data.level === 1 ? '⚡ Difficulté accrue !' : 'Bonne chance !';
        this.add.text(GW / 2, GH / 2 + 10, msg, { fontSize: '26px', fontFamily: 'Courier New', color: '#fff' }).setOrigin(0.5);
    ...
    class GameScene extends Phaser.Scene {
      ...
      create(data) {
        ...
        this.jmpBoost = false;
        this.ptimers = {};
        this._pjump = false;  // previous jump key state (for JustDown on touch)
        ...
        this._makeCoins();
        this._makeSpikes();
        this._makePowerups();   // static group for spawned powerups
        this._makeEnemies();
        this._makeFlag();
        this._makePlayer();
        this._makeHUD();
      ...
      // ─── POWERUPS GROUP ─────────────────────────────────────────
      _makePowerups() {
        this.puGrp = this.physics.add.staticGroup(); // powerups spawn into this
      }
      ...
      _spawnPU(x, y, type) {
        ...
        const pu = this.puGrp.create(x, y - T, 'coin'/* temp */).setTexture(key).setOrigin(0.5, 0.5);
      ...
      _setupCollisions() {
        ...
        // Player ↔ bricks (hit from below = break)
        ...
        // Player ↔ ? blocks (hit from below = activate)
        ...
        this.physics.add.overlap(p, this.enemyGrp, (player, enemy) => {
          ...
          // Stomp detection: player falling, feet above enemy center
      ...
      _updatePlayer(dt) {
        ...
        // JustDown equivalent for touch (avoids holding jump)
        ...
        if (jumpNow && onGround) { p.setVelocityY(jmp); SFX.jump(); }
      ...
      _updateEnemies(dt) {
        ...
        this.enemyGrp.children.each(e => {
          ...
          if (e.etype === 'k') {
            ...
            // Goomba: patrol
            if (Math.abs(e.body.velocity.x) < 5) e.setVelocityX(60 * e.dir);
            else e.setVelocityX(60 * e.dir);
          }
